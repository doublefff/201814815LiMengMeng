...
def preprocessor():
    post_documents=[]
    i=0
    if not os.path.exists('data/out/post_documents_lemmated_5.csv'):
        for doc in documents:
            ...
            # 过滤词频
            back = list(filter(lambda token: str(lemmated).count(token) >= 5, lemmated))

            post_documents.append(back)
            docs = [str(doc) for doc in post_documents]
            pd.DataFrame(docs).to_csv('data/out/post_documents_lemmated_5.csv', sep=" ", header=None, index=None)
        else:
            post_documents=np.array(pd.read_csv('data/out/post_documents_lemmated_5.csv', sep=" ", header=None))
    return post_documents

# 返回字典
def dict_create(post_documents):
    s=set()
    if not os.path.exists('data/out/dictionary_lemmated_5.csv'):
        i=0
        for doc in post_documents:
            ...
            print(i)
        print(len(s))
        pd.DataFrame(list(s)).to_csv('data/out/dictionary_lemmated_5.csv', sep=" ", header=None, index=None)
    else:
        s = np.array(pd.read_csv('data/out/dictionary_lemmated_5.csv', sep=" ", header=None)).reshape(1, -1)[0]
        print(len(s))
    return s
...
def vsm(post_documents,dictionary):
    ...
    for document in post_documents:
        ...
        i+=1
        print(i)
    pd.DataFrame(vectors).to_csv('data/out/vsm-01_lemmated_5.csv', sep=",", header=None, index=None)
